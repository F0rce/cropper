...
/**
 * Define settings for the {@link Cropper}, which can be applied via the {@link
 * Cropper#Cropper(CropperSettings, String, String)} constructor.
 *
...
public class CropperSettings {
  ...
  /**
   * Define the view mode of the cropper. If you set <code>viewMode</code> to {@link ViewMode#ZERO}
   * the crop box can extend outside the canvas, while a value of {@link ViewMode#ONE}, {@link
   * ViewMode#TWO} or {@link ViewMode#THREE} will restrict the crop box to the size of the canvas.
   * <code>viewMode</code> of {@link ViewMode#TWO} or {@link ViewMode#THREE} will additionally
   * restrict the canvas to the container. There is no difference between {@link ViewMode#TWO} and
   * {@link ViewMode#THREE} when the proportions of the canvas and the container are the same. <br>
   * <br>
   * <strong>Default:</strong> {@link ViewMode#ZERO} <br>
   * <br>
   * <strong>Options:</strong><br>
   * - {@link ViewMode#ZERO}: no restrictions<br>
   * - {@link ViewMode#ONE}: restrict the crop box not to exceed the size of the canvas.<br>
   * - {@link ViewMode#TWO}: restrict the minimum canvas size to fit within the container. If the
   * proportions of the canvas and the container differ, the minimum canvas will be surrounded by
   * extra space in one of the dimensions.<br>
   * - {@link ViewMode#THREE}: restrict the minimum canvas size to fill fit the container. If the
   * proportions of the canvas and the container are different, the container will not be able to
   * fit the whole canvas in one of the dimensions.
   *
   ...
   * of the canvas (image wrapper). <br>
   * <br>
   * <strong>Default:</strong> <code>0</code> (interpreted as null/NaN by frontend)<br>
   * <br>
   * Only availible when the {@link #setAspectRatio(double)} option is set to 0.<br>
   * <br>
  ...
  /**
   * Define the fixed aspect ratio of the crop box. By default, the crop box has a free ratio. <br>
   * <br>
   * <strong>Default:</strong> <code>0</code> (interpreted as null/NaN by frontend)<br>
   * <br>
   * <strong>Example:</strong><br>
   ...
   * <br>
   * If so, a <code>crossOrigin</code> attribute will be added to the cloned image element, and a
   * timestamp parameter will be added to the <code>src</code> attribute to reload the source image
   * to avoid browser cache error.<br>
   * <br>
   * Adding a <code>crossOrigin</code> attribute to the image element will stop adding a timestamp
   * to the image URL and stop reloading the image. But the request (XMLHttpRequest) to read the
   * image data for orientation checking will require a timestamp to bust cache to avoid browser
   * cache error. You can set the {@link #setCheckOrientation(boolean)} option to <code>false</code>
   * to cancel this request.<br>
   * <br>
   * If the value of the image's <code>crossOrigin</code> attribute is <code>"use-credentials"
   * </code>, then the <code>withCredentials</code> attribute will set to <code>true</code> when
   * read the image data by XMLHttpRequest.<br>
   * <br>
   * <strong>Default:</strong> <code>true</code>
   *
  ...
  /**
   * Check the current image's Exif Orientation information. Note that only a JPEG image may contain
   * Exif Orientation information.<br>
   * <br>
   * Exactly, read the Orientation value for rotating or flipping the image, and then override the
   * Orientation value with 1 (the default value) to avoid some issues on iOS devices.<br>
   * <br>
   * Requires to set both the {@link #setRotatable(boolean)} and {@link #setScalable(boolean)}
   * options to <code>true</code> at the same time.<br>
   * <br>
   * <strong>Note:</strong> Do not trust this all the time as some JPG images may have incorrect
   * (non-standard) Orientation values<br>
   * <br>
   * <strong>Default:</strong> <code>true</code>
   *
  ...
  /**
   * Enable to toggle drag mode between <code>"crop"</code> and <code>"move"</code> when clicking
   * twice on the cropper.<br>
   * <br>
   * <strong>Default:</strong> <code>true</code>
   *
  ...
  /**
   * Turns the settings into an JSON Object which will be sent to the frontend as soon as cropper
   * gets initialized.
   *
   * @return {@link String}
   */
  public String toJSON() {
    return "{ \"viewMode\": "
        ...
        + ", \"toogleDragModeOnDblclick\": "
        + this.toogleDragModeOnDblclick
        + ", \"minContainerWidth\": "
